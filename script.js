...
function startGame() { // Denne forteller at spillet starter "onload", se helt nederst i scriptet. Med 20 sekunder.
    clearInterval(timer);
    seconds = 20;
    timer = setInterval(countSeconds, 1000);
}

function countSeconds() { // Denne funksjonen forteller at spillet teller ned. Og når den er på 0 så er Tamagochi død xD .
    if (seconds > 0) {
...
function showMessageAndRestart(message) { // Denne funksjonen forteller at en beskjed skal bli gitt når man trykker på knappene. Se under.
    clearInterval(timer);
